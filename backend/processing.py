...
def extract_text_from_docx(file_path: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        # Merge short paragraphs into logical chunks for better TTS experience
        current_chunk = ""
        chunk_idx = 0
...
def extract_text_from_html(file_path: str) -> List[Dict[str, str]]:
    ...
    try:
        ...
        # Try to find main content or just get paragraphs
        ...
        # Capture paragraphs, headers AND divs that likely contain text
        paragraphs = main_content.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'div'])
        
        sections = []
        for i, p in enumerate(paragraphs):
            # For divs, only take them if they don't have many children (likely a text block)
        ...
        # Fallback if no sections found (aggressive text extraction)
...
def extract_text_from_md(file_path: str) -> List[Dict[str, str]]:
    """
    Extracts text from Markdown files by stripping basic md syntax.
    """
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
            
        # Naive markdown strip (remove bold/italic asterisks, links brackets)
        content = re.sub(r'[*_]{1,3}([^*_]+)[*_]{1,3}', r'\1', content) # Bold/italics
        content = re.sub(r'\[([^\]]+)\]\([^)]+\)', r'\1', content)     # Links
        content = re.sub(r'^[#>\-\*\s]+', '', content, flags=re.MULTILINE) # Headers, lists, blockquotes
